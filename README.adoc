...
<<blob/master/src/framework/lee_asciidoctor.erl,lee_asciidoctor>> module allows to "enrich" the model with the docstrings extracted from an external source,
see <<blob/master/doc/src/schema.adoc,example>>.
